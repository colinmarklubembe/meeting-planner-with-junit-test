...
public class Calendar {
	...
	public Calendar() {
		...
		// by Kato - here we are initializing the 'occupied' 3D array with month lists
		// and each of them have empty day lists(dyas with no meetings)
		for (int i = 0; i <= 13; i++) {
			// Initialize month
		 ...
		 */
		// by Kato - here we have a potential problem
	...
	public boolean isBusy(int month, int day, int start, int end) throws TimeConflictException {
		...
		for (Meeting toCheck : occupied.get(month).get(day)) {
			// by Kato - here is for checking if start falls within the duration of the
			// meeting, toCheck
